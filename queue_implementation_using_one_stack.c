...
void peek_of_queue(int stack[], int top_of_stack, int size_of_stack)
{
    if (top_of_stack == -1)
    {
    ...
    int front_element = stack[top_of_stack--];
    if (top_of_stack == -1)
    {
        printf("peak is %d\n", front_element);
        return;
    }
    peek_of_queue(stack, top_of_stack, size_of_stack);
}
...
int main()
{
    ...
    while (1)
    {
        ...
        {
            peek_of_queue(stack, top, size_of_stack);
        }
        else if (choice == 4)
        {
            is_queue_empty(top);
